...
MAIN_MODEL = "YOLOv11 Segmentation N (Augmented) (Best)"
...
async def lifespan(app: FastAPI):
    ...
    session_data[0] = {
        ...
        "show_boxes": True,
        "show_masks": True,
        "zoom_scale":1.0, #Scale must be >= 1.0. Disable zoom by setting this to 1.0
        "zoom_mode":"center", #Should be "center" or "grid",otherwise no zoom
    ...
    logger.info("Current model: %s",model_dict)
...
async def update_var(session_token:str,option: str, var: str):
    ...
    logger.info("Previous value %s has value %s with type %s",option.replace("-","_"),session_data[session_token].get(option,None),type(session_data[session_token].get(option,None)))
    match option:
        case "conf":
        ...
        case "zoom-scale":
            session_data[session_token]["zoom_scale"] = float(var)
        case "cx":
            session_data[session_token]["cx"] = float(var)
        case "cy":
            session_data[session_token]["cy"] = float(var)
        case "show-boxes":
            session_data[session_token]["show_boxes"] = (var.lower().split("=",1)[1] == "true")
        case "show-masks":
            session_data[session_token]["show_masks"] = (var.lower().split("=",1)[1] == "true")
        case "rescale":
            session_data[session_token]["rescale"] = (var.lower().split("=",1)[1] == "true")
        case "rescale":
            session_data[session_token]["zoom_mode"] = var.lower().split("=",1)[0]
        case _:
            return JSONResponse(status_code=404, content={"message": f"Parameter {option.replace("-","_")} must be in {session_data[session_token].keys()}"})
    logger.info("Current value %s has value %s with type %s",option.replace("-","_"),session_data[session_token][option.replace("-","_")],type(session_data[session_token][option.replace("-","_")]))
...
def infer_image(image,session_token=0):
    def get_box(w,h,session_token=0):
        w_s,h_s = w/session_data[session_token]["zoom_scale"],h/session_data[session_token]["zoom_scale"]
    ...
    results = model[0].track(image,conf=session_data[session_token]["conf"],iou=session_data[session_token]["iou"],device=session_data[session_token]["device"],persist=True)
    #Use 2x zoom if no objects were found
    if not results[0].boxes:
        logger.info("No objects found.")
        if session_data[session_token]["zoom_scale"] > 1.0:
            ...
            results2 = model.get(1,model[0]).track(image2,conf=session_data[session_token]["conf"],iou=session_data[session_token]["iou"],device=session_data[session_token]["device"],persist=True)
            ...
            if session_data[session_token]["rescale"]:
                ...
                def rescale_x(x):
                    return int((x/session_data[session_token]["zoom_scale"])+left_bound)
                def rescale_y(y):
                    return int((y/session_data[session_token]["zoom_scale"])+top_bound)
            ...
            final_img = results2[0].plot(boxes=session_data[session_token]["show_boxes"],masks=session_data[session_token]["show_masks"])
        else:
            final_img = results[0].plot(boxes=session_data[session_token]["show_boxes"],masks=session_data[session_token]["show_masks"])
    else:
